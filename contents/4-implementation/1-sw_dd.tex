...
\begin{enumerate}
  \item \textbf{Different \glsentryshort{HDFS} protocol version}: \gls{HDFS} makes use of a \gls{RPC} protocol to interact with a \gls{HDFS} client. Hdfs-native is based off protocol version 3.2, while \gls{HopsFS} was compatible with version 2.7.
  \item \textbf{No support for \glsentryshort{TLS} in hdfs-native}: hdfs-native security is based off of Kerberos, but it does not secure packet transfers using \gls{TLS}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Adding support for \glsentryshort{TLS} in the hdfs-native library}: \gls{TLS} support to hdfs-native was added via the use of an external Rust library called tokio-rustls version 0.26 \cite{RustlsTokiorustlsAsync}.
\end{enumerate}
